...
export async function listProjects(
  ...
  let q = supabase
    .from('projects')
    .select(buildSelect(params.include))
  ...
  // Badge filter
  if (params.badge)
    q = q.eq('badge', params.badge)

  // Status filter
  if (params.statusList?.length)
    q = q.in('status', params.statusList)

  // Date range
  if (params.from)
    q = q.gte('created_at', params.from)
  if (params.to)
    q = q.lt('created_at', params.to)

  // Contributor filter (requires join table)
  if (params.contributors?.length) {
    q = q
      .select(buildSelect(params.include) + ',project_contributors!inner(user_id)')
