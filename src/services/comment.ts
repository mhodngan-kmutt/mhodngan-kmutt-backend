...
export async function listCommentsByProject(
  ...
  const nextCursor = data.length === limit ? (data[data.length - 1].commented_at ?? undefined) : undefined;
...
export async function deleteOwnComment(
  ...
  const { error } = await supabase.from('comments').delete().eq('comment_id', params.commentId);
  if (error) {
    throw Object.assign(new Error('Failed to delete comment'), { cause: error });
  }
}
